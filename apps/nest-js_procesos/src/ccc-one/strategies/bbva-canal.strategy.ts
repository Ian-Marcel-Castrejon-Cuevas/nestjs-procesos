...
export class BbvaCanalStrategy extends BaseCanalStrategy {
  ...
  async descargarTrabajos(
    ...
    while (pagina <= maxPaginas) {
      this.logger.info(` [BBVA] Revisando página ${pagina}...`);
        ...
        for (let i = 0; i < rowCount; i++) {
          try {
            ...
            if (
              ...
              this.logger.info(
                ` [BBVA] Descargando fila ${i + 1}: ${fechaDesde} - ${fechaHasta}`,
              );
              ...
              if (rutaDescarga) {
                ...
                if (resultado) {
                  ...
                  this.logger.info(`[BBVA] Archivo procesado correctamente`);
                }
              }
            }
          } catch (rowError) {
