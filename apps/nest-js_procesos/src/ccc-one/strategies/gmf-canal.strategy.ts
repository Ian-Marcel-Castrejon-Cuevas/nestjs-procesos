...
export class GmfCanalStrategy extends BaseCanalStrategy {
  ...
  async descargarTrabajos(
    ...
    while (pagina <= maxPaginas) {
      this.logger.info(` [GMF] Revisando página ${pagina}...`);
        ...
        for (let i = 0; i < rowCount; i++) {
          try {
            ...
            if (
              ...
              this.logger.info(
                ` [GMF] Descargando fila ${i + 1}: ${fechaDesde} - ${fechaHasta}`,
              );
              ...
              if (rutaDescarga) {
                ...
                if (resultado) {
                  ...
                  this.logger.info(` [GMF] Archivo procesado correctamente`);
                }
              }
            }
          } catch (rowError) {
