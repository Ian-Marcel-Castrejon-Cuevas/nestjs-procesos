...
@Injectable()
/**
 * Estrategia para el canal ATT: navegación, filtros y descarga específicos.
 */
export class AttCanalStrategy extends BaseCanalStrategy {
  constructor(
    logger: ReportLoggerService,
    private storageService: CccOneStorageService,
  ) {
    super('0002' as Canal, logger);
  }
  ...
  async descargarTrabajos(
    ...
    while (pagina <= maxPaginas) {
      this.logger.info(` [ATT] Revisando página ${pagina}...`);
        ...
        for (let i = 0; i < rowCount; i++) {
          try {
            ...
            if (
              ...
              this.logger.info(
                `[ATT] Descargando fila ${i + 1}: ${fechaDesde} - ${fechaHasta}`,
              );
              ...
              if (rutaDescarga) {
                ...
                if (resultado) {
                  ...
                  this.logger.info(` [ATT] Archivo procesado correctamente`);
                }
              }
            }
          } catch (rowError) {
